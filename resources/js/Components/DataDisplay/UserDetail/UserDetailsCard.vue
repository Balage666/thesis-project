        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-sm-12 d-grid gap-2 gap-lg-0 d-lg-flex d-md-flex align-items-md-center align-items-lg-center justify-content-md-center justify-content-lg-between">
                    ...
                    <button data-cy="toggle-edit-mode" v-show="!permissions.has_only_customer_role && ( ( permissions.has_moderator_role_at_most && !targetUserRoles.includes('Moderator') ) || permissions.has_admin_role) || currentUser.id == props.user.id " @click="emitToggleData" class="btn btn-lg btn-info">{{ __("Edit Mode") }}</button>
                </div>
            </div>
        </div>
    </div>
